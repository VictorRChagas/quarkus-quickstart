...
* `PriceProducer` - the [PriceProducer](src/main/java/queue/PriceProducer.java) sends random prices to a JMS queue.
* `PriceConsumer` - the [PriceConsumer](src/main/java/queue/PriceConsumer.java) receives the JMS message and stores the last price.
* `PriceResource`  - the [PriceResource](src/main/java/queue/PriceResource.java) gets the latest price from the PriceConsumer and returns it to the browser.
